...
| R-ROLLBACK | ApplyAgent writes only APPROVED fixes + rollback journal (one-command revert) | `src/catalogguard/agents/apply.py`, `src/catalogguard/storage/rollback.py` | `tests/unit/test_rollback.py` | — | — | done |
| R-HITL | FastAPI review API + HTMX UI, approval state in SQLite | `api/`, `ui/`, `src/catalogguard/storage/` | `tests/integration/test_review_api.py` | — | ADR-001 | done |
